# This Makefile is for the WWW::Mechanize::DecodedContent extension to perl.
#
...
#     ABSTRACT => q[decode Mech content using its HTTP response encoding]
#     AUTHOR => q[Tatsuhiko Miyagawa <miyagawa@bulknews.net>]
#     DIR => []
#     DISTNAME => q[WWW-Mechanize-DecodedContent]
#     NAME => q[WWW::Mechanize::DecodedContent]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], WWW::Mechanize=>q[0], HTTP::Response::Encoding=>q[0] }
#     VERSION => q[0.01]
...
NAME_SYM = WWW_Mechanize_DecodedContent
VERSION = 0.01
...
FULLEXT = WWW/Mechanize/DecodedContent
BASEEXT = DecodedContent
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/WWW/Mechanize/DecodedContent.pm
...
TO_INST_PM = lib/WWW/Mechanize/DecodedContent.pm

PM_TO_BLIB = lib/WWW/Mechanize/DecodedContent.pm \
...
DISTNAME = WWW-Mechanize-DecodedContent
DISTVNAME = WWW-Mechanize-DecodedContent-0.01
...
manifypods : pure_all  \
	lib/WWW/Mechanize/DecodedContent.pm \
	lib/WWW/Mechanize/DecodedContent.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/WWW/Mechanize/DecodedContent.pm $(INST_MAN3DIR)/WWW::Mechanize::DecodedContent.$(MAN3EXT) 
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>decode Mech content using its HTTP response encoding</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Tatsuhiko Miyagawa &lt;miyagawa@bulknews.net&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="HTTP-Response-Encoding" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/WWW/Mechanize/DecodedContent.pm blib/lib/WWW/Mechanize/DecodedContent.pm 
